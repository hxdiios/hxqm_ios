...
- (BOOL)saveFormData:(NSMutableDictionary *)data{
    ...
    if([@"FQ" isEqualToString:state]){
        ...
        ProblemManager *problemManager = [[ProblemManager alloc] initWithDb:db];
        NSDictionary *formMap = [problemManager getProblemInfo:boProblemId userid:[AppConfigure objectForKey:@"USERID"]];
        [data setValue:([formMap objectForKey:@"CREATE_TIME"]==nil) ? @"":[formMap objectForKey:@"CREATE_TIME"] forKey:@"PROBLEM_CREATE_TIME"];
