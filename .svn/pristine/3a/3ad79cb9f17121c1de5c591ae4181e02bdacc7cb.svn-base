...
- (void)viewDidLoad {
    ...
    [self addNavRightBtnByTitle:selectedTitle icon:@"icon_group_sort" action:@selector(flowout)];
...
- (void)addNavRightBtnByTitle:(NSString *)title icon:(NSString *)icon action:(SEL)action{
    UIImage *listImage = [UIImage imageNamed:icon];
    // 设置导航栏按钮的点击执行方法等
    UIBarButtonItem *projectListItem = [[UIBarButtonItem alloc] initWithImage:listImage style:UIBarButtonItemStylePlain target:self action:action];
    // 设置按钮的颜色
    projectListItem.tintColor = [UIColor whiteColor];
    self.navigationItem.rightBarButtonItem = projectListItem;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([@"4" isEqualToString:item_type]){
        ...
        rectifyViewController.delegate = self;
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if([@"1" isEqualToString:need_refresh]){
        [self reqData];
        [AppConfigure setValue:@"0" forKey:NEED_REFRESH];
    }
}
