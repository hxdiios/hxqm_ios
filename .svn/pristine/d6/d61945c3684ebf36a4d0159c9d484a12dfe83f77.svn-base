...
//返回输入日期加上输入天数
+ (NSDate *)addDay:(NSDate *)date dayOfMonth:(int)day{
    NSTimeInterval timeInterval = day*24*60*60;
 ...
 *
 *  @param dateTime 输入日期
 *  @param format_input 输入格式
 *  @param format_output 输出格式
 *  @return 格式化后的日期字符串
 */
+ (NSString *)getFormatDateString:(NSString *)dateTime format_input:(NSString *)format_input format_output:(NSString *)format_output{
    if(dateTime == nil || [@"" isEqualToString:dateTime]){
        return @"";
    }
    //实例化NSDateFormatter对象
    NSDateFormatter *outDateFormatter = [[NSDateFormatter alloc] init];
    //设定时间格式,这里可以设置成自己需要的格式
    [outDateFormatter setDateFormat:format_output];
    NSDate *inputDate = [[NSDate alloc]init];
    NSRange range = [dateTime rangeOfString:@":"];
    if(range.length <= 0){
        inputDate = [BaseFunction dateFromString:dateTime dateFormat:@"yyyy-MM-dd"];
    }else{
        inputDate = [BaseFunction dateFromString:dateTime dateFormat:format_input];
    }
    return [outDateFormatter stringFromDate:inputDate];
}
